...
const glassCardClassName = "border-white/60 bg-white/52 shadow-soft backdrop-blur-xl";
const innerCardClassName = "rounded-[1.25rem] border border-white/60 bg-white/72 backdrop-blur-md";

async function getMetrics() {
  return getDashboardMetrics();
}

export async function DashboardHeroCards() {
  ...
  return (
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {cards.map((card) => {
        ...
        return (
          <Card key={card.title} className={glassCardClassName}>
...
export async function DashboardDailySummary() {
  const metrics = await getMetrics();
...
export async function DashboardCommercialProgress() {
  const metrics = await getMetrics();
    ...
    <section>
      <Card className={glassCardClassName}>
        ...
        <CardContent className="space-y-5">
          <div className="rounded-[1.5rem] border border-white/60 bg-gradient-to-r from-white/50 via-cyan-50/75 to-white/45 p-5 backdrop-blur-md">
...
export async function DashboardAnalyticsSection() {
  ...
  const reportFilters = ["Todos", "Profissional", "Plano", "Tratamento", "Especialidade"];
  const periodFilters = ["Hoje", "Semana", "Mes", "30 dias", "90 dias"];
    ...
    <section className="grid gap-4 xl:grid-cols-[1.35fr_0.95fr]">
      <Card className={glassCardClassName}>
          ...
          <div className="flex flex-wrap gap-2">
            {reportFilters.map((filter, index) => (
              <span
                ...
                className={cn(
                  "rounded-full border px-3 py-1.5 text-xs font-medium",
                  index === 0 ? "border-transparent bg-cyan-100 text-primary" : "border-white/60 bg-white/65 text-slate-600"
                )}
              >
                {filter}
              </span>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex h-80 items-end gap-3 rounded-[1.5rem] border border-white/60 bg-white/40 p-6 shadow-inner shadow-cyan-100/40 backdrop-blur-md">
        ...
        <CardContent className="grid gap-3 sm:grid-cols-2">
          ...
          <div className={innerCardClassName}>
            <div className="p-4">
              ...
              <p className="mt-1 text-2xl font-bold text-slate-950">{formatCompactNumber(metrics.birthdaysThisMonth)}</p>
            </div>
          </div>
          <div className={innerCardClassName}>
            <div className="p-4">
          ...
          <div className={innerCardClassName}>
            <div className="p-4">
              ...
              <p className="mt-1 text-2xl font-bold text-slate-950">{formatCompactNumber(metrics.newPatientsThisMonth)}</p>
            </div>
          </div>
          <div className={innerCardClassName}>
            <div className="p-4">
...
export async function DashboardGoalSection() {
  const metrics = await getMetrics();
...
export async function DashboardSupportGrid() {
  const metrics = await getMetrics();
    ...
    <section className="grid gap-4 xl:grid-cols-2 2xl:grid-cols-4">
      <Card className={glassCardClassName}>
        ...
        <CardContent className="space-y-3">
          {metrics.attentionItems.length > 0 ? (
            ...
            <div className="rounded-[1.25rem] border border-dashed border-white/70 bg-white/40 p-4 text-sm text-slate-500">
      ...
      <Card className={glassCardClassName}>
        <CardHeader className="space-y-2">
          ...
          <CardDescription>Consultas carregadas de forma independente para manter a home sempre responsiva.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          {metrics.appointments.length > 0 ? (
            metrics.appointments.map((appointment) => (
              <div key={appointment.id} className={innerCardClassName}>
                <div className="flex items-start justify-between gap-3 p-4">
                  <div>
                    ...
                    <p className="mt-2 font-medium text-slate-800">{appointment.patientName}</p>
        ...
        <CardContent className="space-y-3">
          {metrics.conversationsQueue.length > 0 ? (
            metrics.conversationsQueue.map((conversation) => (
              <div key={conversation.id} className={innerCardClassName}>
                <div className="flex items-start justify-between gap-3 p-4">
                  <div className="space-y-1">
              ...
              <div key={item.id} className={innerCardClassName}>
                <div className="flex items-start justify-between gap-3 p-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs uppercase tracking-[0.14em] text-slate-400">{item.appointments} atendimento(s)</p>
                  </div>
                  <p className="text-right text-sm font-semibold text-emerald-600">{formatCurrency(item.revenue)}</p>
                </div>
              </div>
            ))
          ) : (
...
export function DashboardSectionSkeleton({
  ...
  return (
    <div className={cn("grid gap-4", cards === 4 && "sm:grid-cols-2 xl:grid-cols-4", cards === 2 && "xl:grid-cols-2")}>
      {Array.from({ length: cards }).map((_, index) => (
        <Skeleton key={index} className={cn("border border-white/60", tall ? "h-[24rem]" : "h-40")} />
      ))}
    </div>
  );
}

export function DashboardWideSkeleton() {
  return <Skeleton className="h-56 border border-white/60" />;
}

export function DashboardGoalSkeleton() {
  return <Skeleton className="h-44 border border-white/60" />;
}

export function DashboardSupportSkeleton() {
  return (
    <div className="grid gap-4 xl:grid-cols-2 2xl:grid-cols-4">
      {Array.from({ length: 4 }).map((_, index) => (
        <Skeleton key={index} className="h-[22rem] border border-white/60" />
      ))}
    </div>
  );
}
